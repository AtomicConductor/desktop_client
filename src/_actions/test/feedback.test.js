...
describe('feedback', () => {
  ...
  describe('when user is no signed in', () => {
    it('does not send account id', async () => {
      const getState = jest.fn().mockReturnValueOnce({ user: { accounts: [] } });
      ...
      await expect(feedback(input, os, nw)(dispatch, getState)).resolves.toBe(undefined);
    });
  });
  ...
  describe('when user is signed in', () => {
    ...
    beforeEach(() => {
      getState = jest.fn().mockReturnValue({
        user: {
          accounts: [
            { id: 123456789, selected: true, email: 'user@email.com' }
          ]
        }
      });
    });
    ...
    it('dispatches failure notification when feedback submission fails', async () => {
      await expect(feedback(input, os, nw)(dispatch, getState)).rejects.toThrow("Can't submit feedback");
    });
  });
});