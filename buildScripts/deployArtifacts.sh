...
if [ "$CI_BRANCH" == "master" ]; then
    ...
    aws s3 cp companion*dmg    s3://${AWS_S3_BUCKET_NAME}/companion/companion-latest-osx-installer.dmg --acl public-read
    aws s3 cp companion*exe    s3://${AWS_S3_BUCKET_NAME}/companion/companion-latest-windows-installer.exe --acl public-read
    aws s3 cp companion*run    s3://${AWS_S3_BUCKET_NAME}/companion/companion-latest-linux-installer.run --acl public-read
